...
const passwordRegex = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{6,}$/; // letters, numbers and at least 1 uppercase
export default {
  username: value => typeof value === 'string' && value.length > user.MIN_USERNAME_LENGTH && value.length < user.MAX_USERNAME_LENGTH,
  password: value => typeof value === 'string' && value.length > user.MIN_PASSWORD_LENGTH && value.length < user.MAX_PASSWORD_LENGTH && value.match(passwordRegex),
